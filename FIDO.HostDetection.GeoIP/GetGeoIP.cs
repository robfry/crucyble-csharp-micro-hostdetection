...
namespace FIDO.HostDetection.GeoIP.Maxmind
{
  public class GetGeoIP
  {
    ...
    private static void RunGeoIP()
    {
      try
      {
        while (true)
        {
          var lFidoReturnValues = GetRabbit.ReceiveNotificationQueue(EventQueue.PrimaryConfig.host, EventQueue.PrimaryConfig.hostdetection.geoip.maxmind.queue, GetRabbitEnum.Maxmind);
          if (lFidoReturnValues.Location == null) continue;
